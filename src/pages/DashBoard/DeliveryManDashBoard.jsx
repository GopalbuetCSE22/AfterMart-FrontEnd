...
function DeliveryManDashBoard() {
  ...
  const fetchDeliveryManData = async () => {
    ...
    try {
      const profileRes = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/deliveryman/profile/${deliveryman_id}`
      );
    ...
    try {
      const availableRes = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/deliveryman/orders/available/${deliveryman_id}`
      );
    ...
    try {
      const underShipmentRes = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/deliveryman/orders/under-shipment/${deliveryman_id}`
      );
    ...
    try {
      const deliveredRes = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/deliveryman/orders/delivered/${deliveryman_id}`
      );
  ...
  const handleAcceptOrder = async (order) => {
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL}/api/deliveryman/acceptpurchase`, {
  ...
  const handleDelivered = async (order) => {
    try {
      ...
      await axios.patch(
        `${import.meta.env.VITE_API_URL}/api/deliveryman/markdelivered/${order.shipment_id}`
      );
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      const deliveryman_id = localStorage.getItem("deliveryman_id");
      await axios.patch(
        `${import.meta.env.VITE_API_URL}/api/deliveryman/changepassword/${deliveryman_id}`,
        {
