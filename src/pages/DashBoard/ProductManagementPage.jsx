...
import { FiArrowLeft, FiSave, FiTrash2, FiPlus, FiMessageSquare, FiEdit } from 'react-icons/fi';
...
function ProductManagementPage() {
    ...
    const handleFileChange = async (e) => {
        ...
        toast.info(`${files.length} image(s) selected (upload not yet implemented)`);
    };
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-950 via-gray-950 to-gray-900 text-white p-6">
            <div className="max-w-5xl mx-auto bg-gray-900 p-6 rounded-xl shadow-lg border border-blue-800/30">
                <div className="flex justify-between items-center mb-6">
                    <button onClick={handleBackClick} className="flex items-center text-blue-400 hover:text-blue-200 transition">
                        <FiArrowLeft className="mr-2" /> Back to Dashboard
                    </button>
                    <div className="flex gap-2">
                        <button
                            onClick={() => setActiveTab('edit')}
                            className={`px-4 py-2 rounded-md flex items-center gap-2 ${activeTab === 'edit' ? 'bg-blue-700 text-white' : 'bg-gray-800 text-blue-300 hover:bg-gray-700'}`}>
                            <FiEdit /> Edit Product
                        </button>
                        <button
                            onClick={() => setActiveTab('chat')}
                            className={`px-4 py-2 rounded-md flex items-center gap-2 ${activeTab === 'chat' ? 'bg-blue-700 text-white' : 'bg-gray-800 text-blue-300 hover:bg-gray-700'}`}>
                ...
                {activeTab === 'edit' && (
                    <div className="space-y-6">
                        ...
                        <div>
                            <label className="block mb-1 text-sm text-blue-300">Description</label>
                            <textarea
                                name="description"
                                rows={4}
                        ...
                        <div>
                            <label className="block mb-1 text-sm text-blue-300">Price (৳)</label>
                            <input
                                type="number"
                                name="price"
                        ...
                        <div>
                            <label className="block mb-1 text-sm text-blue-300">Used For</label>
                            <input
                                name="used_for"
                        ...
                        {/* Images */}
                        <div>
                            <label className="block mb-2 text-lg font-semibold text-blue-300">Images</label>
                            <div className="flex flex-wrap gap-4">
                                {images.length === 0 && <p className="text-gray-400">No images uploaded.</p>}
                                {images.map(img => (
                                    <div key={img.media_id} className="relative group w-32 h-32 border border-blue-700 rounded-md overflow-hidden">
                                        <img src={img.image} alt="Product" className="w-full h-full object-cover" />
                                        <button
                                            onClick={() => handleDeleteImage(img.media_id)}
                                            className="absolute top-1 right-1 bg-red-700 p-1 rounded-full text-white opacity-0 group-hover:opacity-100 transition">
                                            <FiTrash2 size={16} />
                                        </button>
                                    </div>
                                ))}
                                <div
                                    ...
                                    className="w-32 h-32 border-2 border-dashed border-blue-700 flex items-center justify-center cursor-pointer hover:bg-blue-800/30 rounded-md transition"
                                >
                                    <FiPlus size={24} className="text-blue-300" />
                ...
                {activeTab === 'chat' && (
                    <div className="mt-4">
                        {!activeChat ? (
                            <div>
                                <h2 className="text-xl font-semibold text-blue-300 mb-4">Buyers Who Messaged</h2>
                                <ul className="space-y-3">
                                    ...
                                    {conversations.map(conv => (
                                        <li
                                            ...
                                            onClick={() => setActiveChat({ buyerId: conv.buyer_id, conversationId: conv.conversation_id })} // Pass conversationId
                                        >
