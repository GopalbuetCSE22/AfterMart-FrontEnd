...
function UserDashBoard() {
  ...
  const [productConversationsLoading, setProductConversationsLoading] =
    useState(false);
  const [productConversationsError, setProductConversationsError] =
    useState(null);
  ...
  useEffect(() => {
    if (
      (activeSection === "myProducts" || activeSection === "chat") &&
      userId
    ) {
  ...
  useEffect(() => {
    if (activeSection === "bought" && userId) {
      ...
      fetch(`${BASE_URL}/products/boughtProducts/${userId}`)
        ...
        .catch((error) => {
          setBoughtProductsError(
            "Failed to load bought products: " + error.message
          );
          setBoughtProductsLoading(false);
        });
    }
  ...
  const fetchProductConversations = useCallback(
    async (productIdToFetch) => {
      ...
      try {
        ...
        setProductConversationsError(
          "Failed to load conversations for this product."
        );
      } finally {
                ...
                <td className="px-4 py-3 text-gray-400">
                  ...
                  {item.status === "DELIVERED" && (
                    <div className="mt-2 flex gap-2">
                      <button
                        className="text-xs px-2 py-1 bg-blue-700 hover:bg-blue-800 text-white rounded-md shadow"
                        onClick={() =>
                      ...
                      <button
                        className="text-xs px-2 py-1 bg-indigo-700 hover:bg-indigo-800 text-white rounded-md shadow"
                        onClick={() =>
                          setReviewModalConfig({
                            type: "delivery",
      ...
      <ul className="mt-6 space-y-4">
        {notifications.map((note) => (
          ...
          <li
            ...
            className={`p-4 rounded-xl shadow-md border cursor-pointer transition ${note.is_read
              ? "bg-gray-800/80 border-gray-700 text-gray-300 hover:bg-gray-700"
              : "bg-yellow-900/30 border-yellow-600 text-yellow-200 hover:bg-yellow-800/30"
              }`}
          >
            <h4 className="text-lg font-semibold">{note.title}</h4>
            <p className="text-sm mt-1">{note.message || note.description}</p>
            <p className="text-xs mt-2 text-gray-400">
              {new Date(note.created_at).toLocaleString()}
            </p>
          </li>
  ...
  const renderChatContent = () => {
    if (!userId) {
      return (
        <div className="text-red-400 mt-6">
          User ID not found. Cannot load chat.
        </div>
      );
    }
    ...
    if (selectedProductForChat) {
      return (
        <div className="mt-6">
          ...
          {productConversationsLoading && (
            <div className="text-blue-400">Loading conversations...</div>
          )}
          {productConversationsError && (
            <div className="text-red-400">{productConversationsError}</div>
          )}
          {!productConversationsLoading &&
            !productConversationsError &&
            productConversations.length === 0 && (
              <p className="text-gray-400">
                No conversations for this product yet.
              </p>
            )}
    ...
    return (
      <div className="mt-6">
        ...
        {myProductsLoading && (
          <div className="text-blue-400">Loading your products...</div>
        )}
        {myProductsError && (
          <div className="text-red-400">{myProductsError}</div>
        )}
        {!myProductsLoading && !myProductsError && myProducts.length === 0 && (
          <p className="text-gray-400">
            You haven't listed any products yet to receive messages on.
          </p>
        )}
        <ul className="space-y-3">
          {myProducts.map((prod) => (
            ...
            >
              ...
              <span className="text-sm text-purple-400">
                View Conversations{" "}
                <FiMessageSquare className="inline-block ml-1" />
              </span>
            </li>
          ))}
        </ul>
  ...
  const DeliveryManModal = ({ details, onClose }) => {
    if (!details) return null;

    return (
      <div
        className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50 overflow-hidden p-4"
        onClick={onClose} // Close modal when clicking on backdrop
      >
        {/* Modal content area */}
        <div
          className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-8 rounded-lg shadow-2xl w-full max-w-lg relative transform transition-all duration-300 ease-in-out scale-100 opacity-100 animate-fade-in overflow-y-auto max-h-[90vh]"
          onClick={(e) => e.stopPropagation()} // Prevent clicks inside modal from closing it
        >
          {/* Close Button */}
          <button
            className="absolute top-4 right-4 text-gray-400 hover:text-white transition-colors duration-200 text-3xl font-light"
            onClick={onClose}
          ...
          {/* Header */}
          <div className="pt-2 mb-6 border-b border-gray-700 pb-4 text-center">
            <h2 className="text-4xl text-yellow-300 font-extrabold tracking-wide">
              Deliveryman Details
            </h2>
            <p className="text-gray-400 mt-2 text-md">
              Comprehensive information about the delivery personnel.
            </p>
          </div>
          ...
          <div className="text-gray-200 space-y-6">
            {/* Name */}
            <div className="bg-gray-800/60 p-5 rounded-lg flex items-center">
              <span className="material-icons text-yellow-500 mr-4 text-2xl">
                person
              </span>
              <div>
                <span className="font-semibold text-yellow-400 block text-base mb-1">
                  Name
                </span>
            ...
            {/* Example of adding an address field: */}
            {details.address && (
              <div className="bg-gray-800/60 p-5 rounded-lg flex items-center">
                <span className="material-icons text-yellow-500 mr-4 text-2xl">
                  location_on
                ...
                <div>
                  ...
                  <span className="text-xl font-medium">{details.address}</span>
                </div>
              </div>
            )}
          </div>
        ...
        {/* Tailwind CSS @layer utilities for custom animation */}
        <style jsx>{`
            @keyframes fade-in {
              from {
                opacity: 0;
  ...
  const BuyerInfoModel = ({ details, onClose }) => {
    if (!details) return null;
      ...
      >
        ...
        >
          ...
          <div className="pt-2 mb-6 border-b border-gray-700 pb-4 text-center">
            ...
            <p className="text-gray-400 mt-2 text-md">
              Comprehensive information about the Buyer.
            </p>
          </div>
  ...
  const handleNotificationClick = async (note) => {
    // console.log(note.reference_id);
    console.log(note.reference_type);
    console.log(note.notification_id);
    console.log(note.metadata.shipment_id);
    try {
      await axios.patch(
        `${BASE_URL}/notifications/${note.notification_id}/read`
      );
    } catch (error) {
      console.error("Error setting isSeen:", error);
      alert("Failed to mark notification as read.");
    }

    if (
      note.reference_type === "MESSAGE" &&
      note.reference_id &&
      note.user_id &&
      note.sender_id
    ) {
      try {
        // Call API to get the specific conversation based on product_id, buyer_id, and seller_id
        const res = await axios.get(
          `${BASE_URL}/messages/getid/conversation_id`,
          {
            params: {
              productId: note.reference_id,
              // buyerId: note.sender_id,
              // sellerId: note.user_id,
              buyerId: note.user_id,
              sellerId: note.sender_id,
            },
          }
        );
    ...
    } else if (note.reference_type === "SHIPMENT" && note.reference_id) {
      try {
        const res = await axios.get(
          `${BASE_URL}/deliveryman/assignment/${note.metadata.shipment_id}`
        );
        if (res.data) {
          setDeliveryManDetails(res.data);
        } else {
          alert("No deliveryman information found.");
        }
      } catch (error) {
        console.error("Failed to fetch deliveryman info:", error);
        alert("Error fetching deliveryman info.");
      }
    }
    else if (note.reference_type === "PURCHASE" && note.reference_id) {
      console.log("Gopal");
      console.log(note.metadata.buyer_id);
      try {
        const res = await axios.get(
          `${BASE_URL}/users/info/${note.metadata.buyer_id}`
        );
        if (res.data) {
          setBuyerDetails(res.data);
        } else {
          alert("No Buyer information found.");
        }
      } catch (error) {
        console.error("Failed to fetch Buyer info:", error);
        alert("Error fetching Buyer info.");
      }
    }
  };
  ...
  return (
    <>
      {console.log("ReviewModalConfig:", reviewModalConfig)}
      {deliveryManDetails && (
        <DeliveryManModal
          details={deliveryManDetails}
          onClose={() => setDeliveryManDetails(null)}
        />
      )}
      {reviewModalConfig && (
        <ReviewModal
      ...
      <div className="flex min-h-screen bg-gradient-to-br from-blue-950 via-gray-950 to-blue-900">
        {/* Sidebar */}
        <aside className="w-80 bg-gradient-to-b from-blue-950/95 via-gray-900/90 to-gray-800/90 text-white pt-8 pb-4 px-8 space-y-4 fixed h-full shadow-2xl rounded-r-3xl flex flex-col items-center backdrop-blur-2xl border-r border-blue-900/80 z-20">
          {" "}
